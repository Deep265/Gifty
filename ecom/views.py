...
def do_filter(request):
    template = loader.get_template('ecom/index.html')
    prod = None
    if request.method == "POST":
        occasion = request.POST.get('occasion')
        age = int(request.POST.get('age'))
        type = request.POST.get('type')
        print(occasion,age,type)
        prod = models.Product.objects.filter(occasion=occasion,age=age,type=type)
        print(prod)
        return HttpResponse(template.render(context={'products': prod}))
